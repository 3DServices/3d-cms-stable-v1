...
import { getAllRoles, getAllPermissions } from "../../api";
...
function mapRbacPermToPermissionSet(p: RbacPermission): PermissionSet {
  return {
    ...
    tenant: p.account_root ?? "engine",
    module: p.permission_module,
    actions: [p.permission_name.split(".")[1] ?? "view"],
    rolesUsing: 0,
    status: "Active",
    lastModified: p.created_at?.slice(0, 10) ?? "",
  };
}
...
export function RbacPage() {
  ...
  const draftItems = [...roles, ...permissionSets, ...MOCK_POLICIES, ...MOCK_TEMPLATES]
  ...
  return (
    <div className="flex flex-1 min-h-0 min-w-0 overflow-hidden relative">
      <main className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="flex flex-col gap-3 p-3">
          ...
          <div className="grid grid-cols-4 gap-3">
            <KpiCard id="ROL-KPI" title="Active Roles" value={rolesLoading ? "..." : String(activeRoles)} delta={`${roles.length} total`} deltaTone="text-[#25D366]" sub="across all tenants" dot="bg-[#25D366]" />
            <KpiCard id="USR-KPI" title="Users Assigned" value={rolesLoading ? "..." : String(totalUsers)} delta="across all roles" deltaTone="text-[#128C7E]" sub="least-privilege enforced" dot="bg-[#128C7E]" />
            <KpiCard id="POL-KPI" title="Active Policies" value={String(activePolicies)} delta={`${MOCK_POLICIES.length} total`} deltaTone="text-[#34B7F1]" sub="governance rules" dot="bg-[#34B7F1]" />
            <KpiCard id="DFT-KPI" title="Pending Drafts" value={String(draftItems)} delta="awaiting approval" deltaTone="text-[#F97316]" sub="maker-checker queue" dot={draftItems > 0 ? "bg-[#FBBF24]" : "bg-[#25D366]"} />
          </div>
          ...
          {sectionTab === "Permission Sets" && (
            permsLoading
              ...
              : <PermissionSetsTable
                  sets={permissionSets.filter(s => !searchQuery || s.name.toLowerCase().includes(searchQuery.toLowerCase()) || s.id.toLowerCase().includes(searchQuery.toLowerCase()))}
                  onSelect={s => { setSelectedPsId(s.id); }}
      ...
      {bladeOpen && selectedRole && (
        <RbacDetailBlade role={selectedRole} onClose={() => setBladeOpen(false)} />
      )}
...
function KpiCard({ id, title, value, delta, deltaTone, sub, dot }: {
  id: string; title: string; value: string; delta: string; deltaTone: string; sub: string; dot: string;
}) {
  return (
